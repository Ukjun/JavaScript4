...
<body>
	<div id="container">
		<div id="right">
			<a
				href="/board/list?page=${param.page }&record_cnt=${param.record_cnt}&search=${param.search}&${param.searchType}"
				class="mar_l">뒤로가기</a>
			<c:if test="${LoginUser.i_user == data.i_user}">
				<a
					href="/board/regmod?page=${param.page }&record_cnt=${param.record_cnt}&search=${param.search}&searchType=${param.searchType}&i_board=${data.i_board}"
					class="mar_l">수정</a>
				<form id="frm"
					action="/board/delete?page=${param.page }&record_cnt=${param.record_cnt}&search=${param.search}&searchType=${param.searchType}"
					method="post">
		...
		<table>
			<tr>
				<th class="s_td">제목</th>
				<td class="l_td" id="elTitle">${data.title}</td>
				<th class="s_td">작성자</th>
				<td class="m2_td" colspan="2"><c:choose>
			...
			<tr>
				<th class="s_td">조회수</th>
				<td class="m_td">${data.hits}</td>
				<th class="s_td">좋아요</th>
				<td class="m_td" colspan="4"><c:if test="${data.yn_like ==0 }">
						<span class="material-icons"
							onclick="toggleChange(${data.yn_like})">favorite_border</span>
					</c:if> <c:if test="${data.yn_like ==1 }">
						<span class="material-icons"
							onclick="toggleChange(${data.yn_like})">favorite</span>
					</c:if></td>
				<c:if test="${data.like_count>0 }">
					<td>
						<div id="id_like" class="pointCursor">
		...
		<div>
			<form id="cmtFrm"
				action="/board/cmt?page=${param.page }&record_cnt=${param.record_cnt}&search=${param.search}&searchType=${param.searchType}"
				method="post">
				...
				<div>
					...
					<input type="submit" id="cmtSubmit" value="입력"> <input
						type="button" value="취소" onclick="clkCmtCancel()">
				</div>
			</form>
		</div>
		<div>
	...
	<script>
		...
		function clkCmtDel(i_cmt){
			if(confirm("Do you want to delete??")){
				location.href ='/board/cmt?page=${param.page }&record_cnt=${param.record_cnt}&search=${param.search}&searchType=${param.searchType}&i_board=${data.i_board}&i_cmt='+i_cmt;
			}
		}
		...
		function toggleChange(yn_like){
			if(yn_like==0){
				alert("좋아요을 누르셨습니다.")
				//쿼리스트링 = 키값 + value 값
				location.href= "/board/toggle?page=${param.page}&record_cnt=${param.record_cnt}&search=${param.search}&searchType=${param.searchType}&i_board=${data.i_board}&yn_like=" + yn_like;
			}
			else if(yn_like==1){
				alert("좋아요를 취소하셨습니다.")
				location.href= "/board/toggle?page=${param.page}&record_cnt=${param.record_cnt}&search=${param.search}&searchType=${param.searchType}&i_board=${data.i_board}&yn_like=" + yn_like;
		 ...
		 function doHighlight(){
			var searchText = '${param.search}';
			var searchType = '${param.searchType}';
			
			switch(searchType) {
        	case 'a': //제목
        		var txt = elTitle.innerText
        		txt = txt.replace(new RegExp('${param.search}'), '<span class="highlight">' + searchText + '</span>');
        		elTitle.innerHTML = txt
        		break;
        	case 'b': //내용
        		var txt = cont_txt.innerText;
        		txt = txt.replace(new RegExp('${param.search}'), '<span class="highlight">' + searchText + '</span>');
        		cont_txt.innerHTML = txt
